...
StructType::StructType(
        ...
        vector<pair<string, Type *>> fields_
) : Type(identify_),
    identify(identify_),
...
FunctionType::FunctionType(
        Type *returns_,
        vector<pair<string, Type *>> parameters_
) : Type("(" +
         joinToString<pair<string, Type *>>(
                 parameters_, ",",
                 [](pair<string, Type *> const &it) { return it.second->name; }
         ) +
         ")->" + returns_->name),
    returns(returns_),
    parameters(std::move(parameters_)) {}

map<string, Type *> &ASTAnalyzer::getSymbolTable() {
...
optional<Type *> ASTAnalyzer::findSymbol(const string &key) {
...
// definitions: vector<pair<string, Type *>>
void ASTAnalyzer::visit_ExtDefList(ASTNode *node) {
    if (node->children.empty()) {
        // ExtDefList: %empty
        node->attributes["definitions"] = vector<pair<string, Type *>>();
        ...
        auto definitions = vector<pair<string, Type *>>();
        auto defA = any_cast<vector<pair<string, Type *>>>(Def->attributes.at("definitions"));
        auto defB = any_cast<vector<pair<string, Type *>>>(DefList->attributes.at("definitions"));
...
void ASTAnalyzer::visit_ExtDef(ASTNode *node) {
    if (node->children.size() == 2 && node->children[1]->name == "SEMI") {
        ...
        auto declaration = any_cast<pair<string, Type *>>(FunDec->attributes.at("declaration"));

        pushSymbolTable();

        auto &table = getSymbolTable();
        auto *functionType = dynamic_cast<FunctionType *>(declaration.second);
...
void ASTAnalyzer::visit_ExtDecList(ASTNode *node) {
    ...
    if (node->children.size() == 1) {
        ...
        auto declaration = any_cast<pair<string, Type *>>(VarDec->attributes.at("declaration"));
        auto definitions = vector<pair<string, Type *>>();
        ...
        auto definitions = vector<pair<string, Type *>>();
        auto defA = any_cast<vector<pair<string, Type *>>>(ExtDecList->attributes.at("definitions"));
...
void ASTAnalyzer::visit_Specifier(ASTNode *node) {
    if (node->children[0]->name == "TYPE") {
        ...
        auto type = any_cast<string>(TYPE->attributes.at("string_value"));

        if (type == "int") {
            node->attributes["type"] = new IntType();
        } else if (type == "float") {
            node->attributes["type"] = new FloatType();
        } else if (type == "char") {
            node->attributes["type"] = new CharType();
        } else if (type == "string") {
            node->attributes["type"] = new StringType();
        } else {
...
void ASTAnalyzer::visit_StructSpecifier(ASTNode *node) {
    if (node->children.size() == 2) {
        ...
        node->attributes["type"] = typeTable[id];
        ...
        auto definitions = any_cast<vector<pair<string, Type *>>>(DefList->attributes.at("definitions"));
        auto *structType = new StructType(id, definitions);
...
// declaration: pair<string, Type *>
void ASTAnalyzer::visit_VarDec(ASTNode *node) {
    Type *type = any_cast<Type *>(node->parent->attributes.at("type"));
    ...
    if (node->children.size() == 1) {
        ...
        node->attributes["declaration"] = pair<string, Type *>{id, type};
        ...
        Type *arrType = new ArrayType(*type, size);

        node->attributes["declaration"] = pair<string, Type *>{id, arrType};
...
void ASTAnalyzer::visit_FunDec(ASTNode *node) {
    auto returnType = any_cast<Type *>(node->parent->attributes.at("return_type"));
    ...
    if (node->children.size() == 3) {
        ...
        auto id = any_cast<string>(ID->attributes.at("string_value"));
        auto *type = new FunctionType(returnType, vector<pair<string, Type *>>());

        node->attributes["declaration"] = pair(id, type);

        if (functionTable.count(id) == 0) {
            functionTable[id] = type;
        } else {
            printf("Error type %d at Line %d: %s",
                   4,
        ...
        auto parameters = any_cast<vector<pair<string, Type *>>>(VarList->attributes.at("parameters"));
        auto *type = new FunctionType(returnType, parameters);
...
void ASTAnalyzer::visit_VarList(ASTNode *node) {
    if (node->children.size() == 1) {
        ...
        auto declaration = any_cast<pair<string, Type *>>(ParamDec->attributes.at("declaration"));

        auto parameters = vector<pair<string, Type *>>();
        ...
        auto paraA = any_cast<vector<pair<string, Type *>>>(VarList->attributes.at("parameters"));
...
void ASTAnalyzer::visit_Stmt(ASTNode *node) {
    ...
    if (node->children.size() == 2 && node->children[0]->name == "Exp") {
        ...
        auto type = any_cast<optional<Type *>>(Exp->attributes.at("type"));
        auto return_type = any_cast<Type *>(node->attributes.at("type"));
        ...
        auto type1 = any_cast<optional<Type *>>(Exp1->attributes.at("type"));
        ...
        auto type2 = any_cast<optional<Type *>>(Exp2->attributes.at("type"));
...
void ASTAnalyzer::visit_DecList(ASTNode *node) {
    ...
    if (node->children.size() == 1) {
        ...
        auto declaration = any_cast<pair<string, Type *>>(Dec->attributes.at("declaration"));
        auto definitions = vector<pair<string, Type *>>();
        ...
        auto definitions = vector<pair<string, Type *>>();
        auto declaration = any_cast<pair<string, Type *>>(Dec->attributes.at("declaration"));
        auto defA = any_cast<vector<pair<string, Type *>>>(DecList->attributes.at("definitions"));
...
void ASTAnalyzer::visit_Dec(ASTNode *node) {
    ...
    if (node->children.size() == 1) {
        ...
        auto type = any_cast<optional<Type *>>(Exp->attributes.at("type"));
        auto require_type = any_cast<Type *>(node->attributes.at("type"));
...
void ASTAnalyzer::visit_Exp(ASTNode *node) {
    ...
    if (node->children.size() == 1 && node->children[0]->name == "ID") {
        ...
        optional<Type *> type = findSymbol(id);
        node->attributes["type"] = type;
        ...
        if (!type1.has_value() || !type2.has_value()) {
            optional<Type *> type = optional<Type *>();
            node->attributes["type"] = type;
            return;
        }
        ...
        FunctionType *type = functionTable.at(id);

        optional<Type *> return_type = type->returns;
        ...
        auto args_type = any_cast<vector<optional<Type *>>>(Args->attributes.at("args_type"));
        ...
        if (dynamic_cast<ArrayType *>(type1.value()) == nullptr) {
            printf("Error type %d at Line %d: %s",
                   10,
                   Exp1->position.first_line,
        ...
        optional<Type *> type = &dynamic_cast<ArrayType *>(type1.value())->type;
        node->attributes["type"] = type;
        node->attributes["lvalue"] = true;
        ...
        auto exp_type = any_cast<optional<Type *>>(Exp->attributes.at("type"));
        ...
        if (dynamic_cast<StructType *>(exp_type.value()) == nullptr) {
            printf("Error type %d at Line %d: %s",
                   13,
                   Exp->position.first_line,
        ...
        auto struct_type = dynamic_cast<StructType *>(exp_type.value());

        Type *find_member = nullptr;
        ...
        optional<Type *> type = find_member;
        node->attributes["type"] = type;
        node->attributes["lvalue"] = true;
...
// args_type: vector<optional<Type *>>
void ASTAnalyzer::visit_Args(ASTNode *node) {
    if (node->children.size() == 1) {
        ...
        auto args_type = vector<optional<Type *>>();
        ...
        auto type = any_cast<optional<Type *>>(Exp->attributes.at("type"));
        auto argsA = any_cast<vector<optional<Type *>>>(Args->attributes.at("args_type"));
